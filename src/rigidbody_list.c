...
Rigidbody_List *new_rigidbody_list() {
  ...
  rigidbody_list->rigid_body = NULL;
...
Rigidbody_List *add_member_to_end_of_list(Rigidbody_List *start,
  ...
  swp->next->prev = swp;
  swp->next->rigid_body = new_rigidbody(target_rectangle, weight);
  return swp->next;
}
...
Rigidbody_List *add_member_to_start_of_list(Rigidbody_List *start,
  ...
  start->prev->rigid_body = new_rigidbody(target_rectangle, weight);
  return start->prev;
}
...
void destroy_rigidbody_list_member(Rigidbody_List *target_member) {
  destroy_rigidbody(target_member->rigid_body);
  free(target_member);
}
